...
RSpec.feature "Infinite scroll", :type => :feature do
    ...
    scenario "User scrolls down the hobby page and posts list will be appended to old posts", js: true do
        create_list(:post, 30, category: create(:category, branch: 'hobby'))
        visit hobby_posts_path
        check_posts_count 
    end

    scenario "User scrolls down the study page and posts list will be appended to old posts", js: true do 
        create_list(:post, 30, category: create(:category, branch: 'study'))
        visit study_posts_path
        check_posts_count
    end

    scenario "User scrolls down team page and posts list will be appended to old posts", js: true do
